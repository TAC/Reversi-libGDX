...
class MainData() {
    ...
    fun getInitialPlaces() = listOf(CENTER_LEFT_UPPER, CENTER_LEFT_UNDER, CENTER_RIGHT_UPPER, CENTER_RIGHT_UNDER)

    fun canPut(place: Place) = (boardStatus[place.x][place.y].stone == Stone.NONE || boardStatus[place.x][place.y].stone == Stone.SELECT) && getCanChangePlaces(place).isNotEmpty()

    fun getAllCanPutPlaces(color: Stone) = boardStatus.flatMap { it }.filter { canPut(Place(it.x, it.y, color)) }

    fun getAllSelectPlaces() = boardStatus.flatMap { it }.filter { it.stone == Stone.SELECT }
